function [ x_post, report] = MSBL( A , Y, options )
%
...
gamma_first    = zeros(Nfreq , Ntheta);
...
for j1 = 1 : options.convergence.maxiter
    ...
    for i_f = 1 : Nfreq
        ...
        gamma_first(i_f,:)   = sum ( abs ( ( ApSigmaYinv * squeeze2(Y(i_f,:,:)) ).^2 ),2 ) / Nsnapshot;
        
        sigma_X_diag = gamma - diag((repmat(gamma, [1 Nsensor] ) .* ApSigmaYinv)*(Af.*repmat(gamma.', [Nsensor 1] )));
    ...
    % Gamma update
    gamma = sum( gamma_first   ,1 ).' + sigma_X_diag ;
    
    %% sigma and L2 error using unbiased posterior update
    
        % locate same peaks for all frequencies
        [ ~ , Ilocs] = findpeaks(abs(gamma),'SORTSTR','descend','NPEAKS',Nsource);
        Apeak      = A(:,:,Ilocs);
...
%     sigc(i_f) =   1/Nsensor * (sum(sum ( abs ( ( squeeze2(Y(i_f,:,:))-Af*ApSigmaYinv * squeeze2(Y(i_f,:,:)) ).^2 ) )) / Nsnapshot + sigc(i_f)*(Ntheta-sum(sigma_X_diag./gamma)));
