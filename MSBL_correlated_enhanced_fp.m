function [x_new,D]=MSBL_correlated_enhanced_fp(Y,A,sigma,eta,inner_it)
...
while iter<iter_mx && norm(mu_new-mu_old)>1e-3
    ...
    for i = 1:inner_it
        ...
        % drawnow
        alpha_d = alpha_d.*(diag(alpha_inv)./diag(update_part));
        alpha_d1 = alpha_d1.*(diag(alpha_inv,1)./diag(update_part,1));
        alpha_d2 = alpha_d2.*(diag(alpha_inv,-1)./diag(update_part,-1));
