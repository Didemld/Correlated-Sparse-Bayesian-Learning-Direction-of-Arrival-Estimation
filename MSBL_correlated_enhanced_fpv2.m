function [x_new,D]=MSBL_correlated_enhanced_fpv2(Y,A,sigma,eta,inner_it)
...
while iter<iter_mx && norm(mu_new-mu_old)>1e-3
    ...
    alpha_d = diag(alpha_all);
    alpha_d1 = diag(alpha_all,1);
    alpha_d2 = diag(alpha_all,-1);
    % if iter==iter_mx && norm(mu_new-mu_old)<1e-3
    % lrate = 0.0001;
    alpha_inv = inv(alpha_all);

    update_part = (alpha_inv*alpha_all*alpha_inv);
    ...
    for i = 1:inner_it
        ...
        %
        % drawnow
        alpha_d = alpha_d.*(diag(update_part)./diag(alpha_inv));
        alpha_d1 = alpha_d1.*(diag(update_part,1)./diag(alpha_inv,1));
        alpha_d2 = alpha_d2.*(diag(update_part,-1)./diag(alpha_inv,-1));
        alpha_up = diag(alpha_d)+diag(alpha_d1,1)+diag(alpha_d2,-1);
